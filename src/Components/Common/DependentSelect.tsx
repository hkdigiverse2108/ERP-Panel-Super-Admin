...
export const DependentSelect = <T extends ApiOption, P = string | undefined>({ params, name, label, grid, required, disabled, enabled = true, query, value, onChange }: DependentSelectProps<T, P>) => {
  ...
  if (value && onChange) {
    return <CommonSelect label={label} disabled={disabled} isLoading={isLoading} options={options} value={value} onChange={onChange} grid={grid} required={required} />;
  }

  return <CommonValidationSelect name={name} label={label} disabled={disabled} isLoading={isLoading} options={options} grid={grid} required={required} />;
};
