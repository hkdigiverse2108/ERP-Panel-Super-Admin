  ...
  return (
    <Grid container spacing={2} sx={{ p: 2 }}>
      ...
      <Grid size={{ xs: 12, md: 9 }} container spacing={2}>
        ...
        {values.createdFrom === "sales-order" && <CommonValidationSelect name="selectedSalesOrderId" label="Reference Sales Order" options={salesOrderOptions} multiple disabled={isEditing || !values.companyId || !values.customerId} isLoading={isSalesOrderLoading || isSalesOrderFetching} grid={{ xs: 12, md: 4 }} />}

        {values.createdFrom === "invoice" && <CommonValidationSelect name="selectedInvoiceId" label="Reference Invoice" options={invoiceOptions} multiple disabled={isEditing || !values.companyId || !values.customerId} isLoading={isInvoiceLoading || isInvoiceFetching} grid={{ xs: 12, md: 4 }} />}
      </Grid>
