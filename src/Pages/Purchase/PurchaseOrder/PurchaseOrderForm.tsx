...
const PurchaseOrderForm = () => {
  ...
  const { data: supplierData, isLoading: supplierDataLoading } = Queries.useGetContactDropdown({ contactType: "supplier" });
                ...
                <Grid container spacing={2}>
                  ...
                  <CommonCard title="Purchase Order Details" grid={{ xs: 12 }}>
                    <Grid container spacing={2} sx={{ p: 2 }}>
                      <CommonValidationSelect name="supplierId" label="Select Supplier" isLoading={supplierDataLoading} options={GenerateOptions(supplierData?.data)} required grid={{ xs: 12, md: 3 }} />
                      <CommonValidationDatePicker name="date" label="Purchase Order Date" grid={{ xs: 12, md: 3 }} required />
                      <CommonValidationTextField name="orderNo" label="Purchase Order No." grid={{ xs: 12, md: 3 }} />
                      <CommonValidationDatePicker name="shippingDate" label="Shipping Date" grid={{ xs: 12, md: 3 }} required />

                      {selectedSupplier && (
                        <Grid  >
                          <Box sx={{ mt: 0.5, display: "flex", flexDirection: "column", gap: 0.5 }}>
                            <Box display="flex" gap={1} flexWrap="wrap">
                              <Box sx={{ color: "#344054", fontWeight: 600, fontSize: "0.875rem" }}>Place of Supply:</Box>
                              <Box sx={{ color: "#667085", fontSize: "0.875rem" }}>{selectedSupplier?.address?.[0]?.state?.name || "-"}</Box>
                            </Box>

                            <Box display="flex" gap={1} flexWrap="wrap">
                              <Box sx={{ color: "#344054", fontWeight: 600, fontSize: "0.875rem" }}>GSTIN:</Box>
                              <Box sx={{ color: "#667085", fontSize: "0.875rem" }}>{selectedSupplier?.address?.[0]?.gstIn || "-"}</Box>
                            </Box>

                            <Box sx={{ mt: 0.5 }}>
                              <Box sx={{ color: "#344054", fontWeight: 600, fontSize: "0.875rem" }}>Billing Address</Box>
                              {selectedSupplier?.address?.length ? (
                                <Box sx={{ color: "#667085", fontSize: "0.875rem" }}>
                                ...
                                <Box sx={{ color: "#667085", fontSize: "0.875rem" }}>Billing Address is Not Provided</Box>
                              )}
                            </Box>
                          </Box>
                        </Grid>
                      ...
                      <CommonValidationTextField name="shippingNote" label="Shipping Note" grid={{ xs: 12, md: 3 }} />
                      <CommonValidationSelect name="taxType" label="Tax Type" options={TAX_TYPE} grid={{ xs: 12, md: 3 }} />
                    </Grid>
                  </CommonCard>
                        ...
                        <>
                          {values.items?.map((_, index) => (
                            <Grid container spacing={2} sx={{ p: 2 }} key={index}>
                              ...
                              <CommonValidationTextField name={`items.${index}.qty`} label="Qty" type="number" required grid={{ xs: 12, md: 3 }} />
                              <CommonValidationTextField name={`items.${index}.unitCost`} label="Unit Cost" type="number" grid={{ xs: 12, md: 3 }} />
