...
export const GenerateOptions = (data?: { _id: string; name?: string; productName?: string; firstName?: string; lastName?: string; title?: string; tabName?: string; fullName?: string; estimateNo?: string }[]) => {
  if (!data || !Array.isArray(data)) return [];

  return data.map((item) => {
    const label = item.name?.trim() || item.fullName?.trim() || item.productName?.trim() || item.estimateNo?.trim() || [item.firstName, item.lastName].filter(Boolean).join(" ") || item.title?.trim() || item.tabName?.trim() || "Unnamed";
